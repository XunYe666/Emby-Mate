...
- 发布快照：[`worker-v2.4.8.js`](./worker-v2.4.8.js)
- 部署配置：[`wrangler.jsonc`](./wrangler.jsonc)

## v2.4.8 发布重点

- 发布仓库继续聚焦单文件部署，保留 `dist/worker.js` 与版本快照两条交付入口。
- 管理后台完成一轮 Kumo-inspired 整理：样式 token、按钮/卡片/分段切换 primitive、移动端 sheet 弹窗与暗色模式细节统一。
- CF 指标口径固定为“近24小时 + 当前域名下已配置站点路径”，卡片和权限说明同步简化，避免旧时间范围文案误导。
- 文档和发布入口同步收口，当前仓库只提供部署所需文件，不公开源码目录。
...
1. 在 Cloudflare 创建 Worker，用 `worker-v2.4.8.js` 走 Dashboard 粘贴部署，或用 `dist/worker.js` 走 Wrangler 工程化部署。
...
2. 创建 Worker，并粘贴 `worker-v2.4.8.js` 全部内容。
...
- 如果只是你自己部署，不需要一键分享，继续用 Cloudflare Dashboard 直接粘贴 `worker-v2.4.8.js` 会更省事。
