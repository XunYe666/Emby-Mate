...
const App={
    ...
    async saveConfig(){
        const proxies=this.settingsDraft.thirdPartyProxies.map(item=>({
        ...
        let tcping;
        try{
            const cfg=cloneTcpingConfig(this.settingsDraft.tcping||{});
            tcping={tcp:toProbe(cfg.tcp,'TCP'),head:toProbe(cfg.head,'HEAD')};
        }catch(e){
            return this.toast(e.message||'延迟测试设置错误','error');
        }

        const sharedRedirectEntries=(Array.isArray(this.settingsDraft.redirectWhitelistEntries)?this.settingsDraft.redirectWhitelistEntries:[]).map(item=>({id:item.id||('redirect-'+Date.now()),name:String(item.name||'').trim(),domain:normalizeSharedRedirectRuleValue(item.domain)})).filter(item=>item.name||item.domain);
        ...
        this.config=buildConfigFromSettingsDraft({...this.config,thirdPartyProxies:proxies,tcping},{...this.settingsDraft,thirdPartyProxies:proxies,tcping,redirectWhitelistEntries:sharedRedirectEntries});
    ...
    async saveCfSettings(){
        const cfMetrics={accountId:(this.cfSettingsDraft?.accountId||'').trim(),apiToken:(this.cfSettingsDraft?.apiToken||'').trim(),workerUrl:(this.cfSettingsDraft?.workerUrl||'').trim(),showCard:this.cfSettingsDraft?.showCard!==false,autoRefreshSeconds:Math.max(30,Math.min(3600,Number(this.cfSettingsDraft?.autoRefreshSeconds)||300))};
